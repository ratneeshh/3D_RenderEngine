...
public class Render extends JPanel {
    ...
    Render() {
        ...
        Vertex v1 = new Vertex(-100, -50, -50);  // Front bottom left
        Vertex v2 = new Vertex(100, -50, -50);   // Front bottom right
        Vertex v3 = new Vertex(100, 50, -50);    // Front top right
        Vertex v4 = new Vertex(-100, 50, -50);   // Front top left
        Vertex v5 = new Vertex(-100, -50, 50);   // Back bottom left
        Vertex v6 = new Vertex(100, -50, 50);    // Back bottom right
        Vertex v7 = new Vertex(100, 50, 50);     // Back top right
        Vertex v8 = new Vertex(-100, 50, 50);    // Back top left

        // Front face (red)
        tris.add(new Triangle(v1, v2, v3, Color.RED));
        tris.add(new Triangle(v1, v3, v4, Color.RED));

        // Back face (green)
        tris.add(new Triangle(v5, v7, v6, Color.GREEN));
        tris.add(new Triangle(v5, v8, v7, Color.GREEN));

        // Top face (blue)
        tris.add(new Triangle(v4, v3, v7, Color.BLUE));
        tris.add(new Triangle(v4, v7, v8, Color.BLUE));

        // Bottom face (yellow)
        tris.add(new Triangle(v1, v6, v2, Color.YELLOW));
        tris.add(new Triangle(v1, v5, v6, Color.YELLOW));

        // Right face (magenta)
        tris.add(new Triangle(v2, v6, v7, Color.MAGENTA));
        tris.add(new Triangle(v2, v7, v3, Color.MAGENTA));

        // Left face (cyan)
        tris.add(new Triangle(v1, v4, v8, Color.CYAN));
        tris.add(new Triangle(v1, v8, v5, Color.CYAN));
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        addMouseWheelListener(e -> {
            ...
            if (e.getWheelRotation() < 0) {
                zoomFactor *= 1.001; // Zoom in
            } else {
                zoomFactor *= 0.999; // Zoom out
            }
            repaint();
        });
